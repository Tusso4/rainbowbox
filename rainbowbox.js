...
function rainbowbox_callback(num, loop=false, elem) {
  ...
  console.log(toBack, toNext, jumps, loop);
  rainbowbox($(elem), toBack, toNext, loop);

  jumps++;
}

function rainbowbox(elem, from=[0, 0, 0], to=[255, 255, 255], loop=false, vel=20) {
	//Vars
    var rgb_now;
    var rgb;
    var count = 0;
    ...
    inter = setInterval(function() {
      if(rgb[0] == rgb_now[0] && rgb[1] == rgb_now[1] && rgb[2] == rgb_now[2]) {
        clearInterval(inter);
        if(to[0].constructor === Array) {
	        rainbowbox_callback(count, loop, elem);
        }
      }
